...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        def new_get_variable(v):
            ...
            if name == 'W' and ('conv0' in name):
                logger.info("Binarizing-8 weight {}".format(v.op.name))
                return fw1(v)
            elif name == 'W' and ('fct' in name):
                logger.info("Binarizing-4 weight {}".format(v.op.name))
                return fw1(v)
            elif name == 'W' and ('conv1' in name or 'conv2' in name or 'conv3' in name or 'conv4' in name):
                logger.info("Binarizing-2 weight {}".format(v.op.name))
                return fw1(v)
            elif name == 'W' and ('fc0' in name or 'fc1' in name):
        ...
        with remap_variables(new_get_variable), \
            ...
            logits = (LinearWrap(image)
                .Conv2D('conv0', 96, 11, stride=4, padding='SAME')
                .apply(fg)
                .BatchNorm('bn0')
                .MaxPooling('pool0', 3, 2, padding='SAME')
                .apply(activate)

                .Conv2D('conv1', 256, 5, stride=2, padding='SAME',split=2)
                .apply(fg)
                .BatchNorm('bn1')
                .MaxPooling('pool1', 3, 2, padding='SAME')
                .apply(activate)

                .Conv2D('conv2', 384, 3, stride=1, padding='SAME',split=2)
                .apply(fg)
                .BatchNorm('bn2')
                .apply(activate)
                ...
                .Conv2D('conv4', 256, 3, stride=1, padding='SAME',split=2)
                .apply(fg)
                .BatchNorm('bn4')
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            ScheduledHyperParamSetter(
                #'learning_rate', [(56, 2e-5), (64, 4e-6), (100, 1e-6)]),
...
if __name__ == '__main__':
    ...
    #config.dataflow = FakeData([[128, 224,224,3],[128]], random=False)
